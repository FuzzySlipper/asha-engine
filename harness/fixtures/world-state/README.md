# World-state snapshot fixtures (Den task #2484)

Committed fixtures for the runtime **world-state snapshot** — the durable artifact
...
checked by `scene-diagnostics`'s `world_state_goldens` test. Regenerate after an
intended change:
...
by `rule-world-bundle`'s `world_state_roundtrip` integration test, which loads a
bundle carrying both a voxel section and this world-state snapshot.
