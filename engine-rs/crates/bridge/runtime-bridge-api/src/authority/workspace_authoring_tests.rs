...
fn authoring_cell_is_distinct_from_gameplay_runtime_and_owns_revisions() {
    ...
    let opened = bridge
        .open_workspace_authoring_authority(open_request("workspace.local"))
        .unwrap();
...
fn project_content_authoring_is_revision_bound_and_promotes_only_the_rust_candidate() {
    let mut bridge = EngineBridge::new();
    let opened = bridge
        .open_workspace_authoring_authority(open_request("workspace.project-content"))
        .unwrap();
...
fn project_content_authoring_rejects_a_source_path_owned_by_another_document() {
    let mut bridge = EngineBridge::new();
    let opened = bridge
        .open_workspace_authoring_authority(open_request("workspace.project-content-paths"))
        .unwrap();
...
fn project_write_is_rust_derived_revision_bound_and_single_use() {
    let mut bridge = EngineBridge::new();
    let opened = bridge
        .open_workspace_authoring_authority(open_request("workspace.project-write"))
        .unwrap();
...
fn procedural_materialization_is_preview_pure_candidate_bound_and_combined_saveable() {
    let mut bridge = EngineBridge::new();
    let opened = bridge
        .open_workspace_authoring_authority(open_request("workspace.procedural"))
        .unwrap();
    let decoded = bridge
        .decode_scene_document(SceneDocumentDecodeRequestDto {
...
fn reopened_procedural_environment_replaces_from_loaded_asset_provenance() {
    ...
    let source_opened = source_bridge
        .open_workspace_authoring_authority(open_request("workspace.procedural-source"))
        .unwrap();
    ...
    let mut bridge = EngineBridge::new();
    let opened = bridge
        .open_workspace_authoring_authority(open_request("workspace.procedural-reopen"))
        .unwrap();
    let decoded = bridge
        .decode_scene_document(SceneDocumentDecodeRequestDto {
...
fn procedural_materialization_rejects_stale_unresolved_and_oversized_without_mutation() {
    let mut bridge = EngineBridge::new();
    let opened = bridge
        .open_workspace_authoring_authority(open_request("workspace.procedural-rejection"))
        .unwrap();
    let decoded = bridge
        .decode_scene_document(SceneDocumentDecodeRequestDto {
            source_text: core_scene::encode(&procedural_scene(42)),
