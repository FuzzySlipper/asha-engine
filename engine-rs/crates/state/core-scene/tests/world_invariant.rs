...
//!
//! `WorldState` is the spatial scene-runtime world: every live entity it holds has
//! a transform capability, enforced by construction. These tests prove that across
//! the public surface — so `WorldState::hash` never feeds a transform-less record
...
use core_scene::{
    ...
    SceneNodeRecord, SceneTransform, WorldState,
};
...
fn assert_spatial_invariant(world: &WorldState) {
...
fn runtime_created_entities_always_have_a_transform() {
    let mut world = WorldState::empty(WorldId::new(1));
...
fn bootstrapped_world_satisfies_the_invariant() {
    ...
    assert_eq!(world.hash(), record.world_hash);
}

#[test]
fn empty_world_hash_does_not_panic_and_is_stable() {
    let a = WorldState::empty(WorldId::new(42));
    let b = WorldState::empty(WorldId::new(42));
    assert_eq!(a.hash(), b.hash());
    // A different world id yields a different fingerprint.
    assert_ne!(a.hash(), WorldState::empty(WorldId::new(43)).hash());
}

#[test]
fn set_transform_preserves_the_invariant_and_changes_the_hash() {
    let mut world = WorldState::empty(WorldId::new(2));
