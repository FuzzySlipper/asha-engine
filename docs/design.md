...
status: current
audience: all
tags: [architecture, design, authority, expression]
supersedes: []
see-also: []
---
...
The repository structure is described in `README.md` and enforced by `governance/ownership.toml`.
The canonical architecture is this document; the current implementation status is tracked in Den.
...
## 13. Historical prototype plan

The original prototype phase plan is archived in Den legacy documents (`asha/legacy/*`).
Current implementation status is tracked in Den tasks/docs; the launchable voxel loop and
post-launchable expansion work are indexed from `README.md` and `docs/topics/authority/voxel.md`.
