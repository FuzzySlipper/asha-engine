...
- `submitRuntimeActionIntent(envelope)`: accepts a typed `RuntimeActionIntentEnvelope` proposal. Rust-backed sessions resolve accepted `primary_fire` pressed intents from the current bridge-owned snapshot of the selected camera, route that authoritative origin/forward ray through the Rust bridge, and return combat/fire/health provenance. Callers select a camera but do not attest target, range, or damage. Reference sessions return labelled fixture/reference receipts. Unsupported actions and unknown camera handles fail closed before combat mutation.
