...
- **Rust internals deliberately were not churned.** `WorldState` still appears in 23 crates vs `SessionState` in 2, and `protocol-world-bundle` keeps its name — consistent with the ADR's "do not churn existing code names blindly" instruction and covered by the compatibility doc.
