...
import { RenderProjection, RenderProjectionError, createGeneratedTunnelRoomFrame, createGeneratedTunnelViewportFrame, } from './index.js';
const repoRoot = resolve(import.meta.dirname, '../../../..');
function fixturePath(name) {
    return resolve(repoRoot, 'harness/fixtures/render-projection', name);
}
function renderDiffFixturePath(name) {
...
function tinyGeneratedTunnelReadout() {
    return {
        generator: {
...
function createPrimitive(handle, label = `node-${handle}`, parent = null) {
    return {
        op: 'create',
...
function meshAsset(asset = 'mesh/crate') {
    return {
        asset,
        payload: quadPayload(),
...
function animatedMeshAsset(asset = 'mesh-animation/kenney-retro-character-medium') {
    return {
        asset,
        runtimeFormat: 'glb',
...
function sprite(asset = 'sprite/ui', frame = 0) {
    return {
        asset,
        frame,
        pivot: [0.5, 0.5],
...
void test('tracks static mesh definitions and fails closed on in-use redefinition', () => {
    ...
    assert.throws(() => projection.applyDiff({ op: 'defineStaticMesh', asset: meshAsset() }), RenderProjectionError);
...
void test('tracks animated mesh definitions and command-selected named clip playback', () => {
    ...
    assert.throws(() => projection.applyDiff({ op: 'defineAnimatedMesh', asset: animatedMeshAsset() }), RenderProjectionError);
...
void test('animated mesh render-diff fixture registers asset and starts the run clip', () => {
    const frame = JSON.parse(readFileSync(renderDiffFixturePath('animated-mesh.json'), 'utf8'));
    const projection = new RenderProjection();
...
void test('fails closed on unknown handles, unsupported ops, and malformed mesh payloads', () => {
    ...
    assert.throws(() => projection.applyDiff({ op: 'teleport', handle: renderHandle(1) }), RenderProjectionError);
...
void test('retained projection fixture matches the committed before/after goldens', () => {
    const frames = JSON.parse(readFileSync(fixturePath('retained-sequence.json'), 'utf8'));
    const before = JSON.parse(readFileSync(goldenPath('retained-sequence.before.json'), 'utf8'));
    const after = JSON.parse(readFileSync(goldenPath('retained-sequence.after.json'), 'utf8'));
    const projection = new RenderProjection();
...
function renderFrameSignature(frame) {
