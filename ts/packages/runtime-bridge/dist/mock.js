...
function materialDescriptor(id, material) {
    const color = [material.render.color.r, material.render.color.g, material.render.color.b, material.render.color.a];
    return {
        schemaVersion: 2,
        id,
        color,
...
export class MockRuntimeBridge {
    ...
    configureInputSession(request) { return this.#inputSession.configure(request); }
    applyInputContextCommand(command) { return this.#inputSession.applyContextCommand(command); }
    submitRawInput(sample) { return this.#inputSession.resolve(sample); }
    replayResolvedInputAction(record) { return this.#inputSession.replay(record); }
    readInputContextState() { return this.#inputSession.readContextState(); }
    applyTimeControlCommand(command) { return this.#timeController.apply(command); }
    readTimeControlState() { return this.#timeController.read(); }
    ...
    applyFpsPrimaryFire(request) {
        ...
        const weapon = shooter.weapon ?? (shooterRole === 'enemy' && targetRole === 'player' ? enemyPolicyWeapon : null);
        if (weapon === null) {
        ...
        const lifecycleStatus = targetRole === 'enemy' && after !== null && after.current === 0
            ? { state: 'enemy_defeated', entity: target.entity, tick }
    ...
    importVoxelConversionMeshSource(_request) {
        void _request;
        if (this.#engine === null) {
    ...
    readVoxelConversionSourceMetadata(_request) {
        void _request;
        if (this.#engine === null) {
    ...
    initializeVoxelVolumeAuthoring(_request) { void _request; return this.#unsupportedAfterInit('initializeVoxelVolumeAuthoring', 'mock bridge does not own voxel volume authoring initialization authority'); }
    ...
    unloadVoxelVolumeAsset(_request) { void _request; return this.#unsupportedAfterInit('unloadVoxelVolumeAsset', 'mock bridge does not own voxel volume asset unload authority'); }
    ...
    applyCameraModeCommand(command) {
        ...
        if (receipt.accepted)
            this.#cameras.set(command.camera, receipt.after.snapshot);
        return receipt;
    }
    applyCameraNavigationInput(input) {
        ...
        if (receipt.accepted)
            this.#cameras.set(input.camera, receipt.after.snapshot);
        return receipt;
    }
