...
function decodeUvStrategy(v, path) {
    switch (v) {
        case 'flat':
        case 'planar':
        case 'atlas':
...
export function decodeMaterialDescriptor(v, path = '$') {
    const o = asObject(v, path);
...
function decodeTextureWrap(v, path) {
    if (v === 'clamp' || v === 'repeat')
        return v;
...
export function decodeTextureDescriptor(v, path = '$') {
    const o = asObject(v, path);
    if (typeof o.id !== 'string' || o.id.length === 0) {
...
function decodeFrameRect(v, path) {
    const o = asObject(v, path);
    const rect = {
...
export function decodeSpriteAtlas(v, path = '$') {
    const o = asObject(v, path);
    if (typeof o.id !== 'string' || o.id.length === 0) {
    ...
    const frames = asArray(o.frames, `${path}.frames`).map((fr, i) => decodeFrameRect(fr, `${path}.frames[${i}]`));
    if (frames.length === 0) {
