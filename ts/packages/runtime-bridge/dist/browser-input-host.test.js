...
import { BrowserFpsResolvedActionConsumer, BrowserInputHost, createDefaultBrowserInputCatalog, createRuntimeSessionFacade, ResolvedPauseContextConsumer, } from './index.js';
import { MockRuntimeBridge } from './mock.js';
function createHost(initialContexts = ['gameplay']) {
...
void test('menu and dialog contexts consume gameplay while preserving their own UI actions', () => {
    ...
    assert.deepEqual(host.readout().recentDeliveries.at(1)?.activeContexts, ['gameplay', 'menu'], 'a later context push does not rewrite the delivery-time snapshot');
});
void test('resolved pause context records and replays without browser events or double delivery', () => {
    ...
    assert.deepEqual(replaySession.readInputContextState().activeContexts.map((context) => context.contextId), ['gameplay']);
...
//# sourceMappingURL=browser-input-host.test.js.map