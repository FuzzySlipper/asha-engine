...
void test('RuntimeSession runs deterministic autonomous enemy policy ticks through typed proposals', () => {
    ...
    assert.equal(session.readTelemetry().replayRecords.some((record) => record.kind === 'lifecycleDeath'), false);
    const movedEnemy = session.readEcrpRuntimeReadout().entities.find((entity) => entity.definitionStableId === 'actor/generated-tunnel-enemy');
