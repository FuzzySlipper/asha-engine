...
const OPERATIONS = new Map(MANIFEST_OPERATIONS.map((operation) => [operation.manifestName, operation]));
function wireError(operation, direction, kind, path, detail, detailCode) {
...
function stringifyWireValue(value) {
    return JSON.stringify(value, (_key, nestedValue) => nestedValue instanceof Uint8Array ? Array.from(nestedValue) : nestedValue);
}
function parseJson(operation, direction, payload, maxBytes) {
    const actualBytes = byteLength(payload);
    if (actualBytes > maxBytes) {
        throw wireError(operation, direction, direction === 'input' ? 'invalid_input' : 'internal', '$', `payload has ${actualBytes} bytes; limit is ${maxBytes}`, 'payload_too_large');
    }
    try {
        return JSON.parse(payload);
    }
    catch {
        throw wireError(operation, direction, direction === 'input' ? 'invalid_input' : 'internal', '$', 'payload is not valid JSON', 'invalid_json');
    }
}
function fieldMatches(kind, value) {
    if (kind === 'nullable_boolean')
        return value === null || typeof value === 'boolean';
    if (kind === 'nullable_number')
        return value === null || (typeof value === 'number' && Number.isFinite(value));
    if (kind === 'nullable_object')
...
function validateCustom(operation, direction, reference, value) {
    const shape = CUSTOM_WIRE_SHAPES[reference.name];
    if (shape === undefined) {
        throw wireError(operation, direction, 'operation_unimplemented', '$', `custom wire validator '${reference.name}' is not registered`, 'missing_custom_validator');
    }
...
function validateReference(operation, direction, reference, value) {
    ...
    const result = validateGeneratedWireValue(reference.name, value);
    if (!result.valid) {
        throw wireError(operation, direction, direction === 'input' ? 'invalid_input' : 'internal', result.issue.path, result.issue.message, result.issue.code);
    }
}
export function serializeOperationInput(operation, value) {
...
export function validateOperationInput(operation, value) {
    serializeOperationInput(operation, value);
}
export function parseOperationOutput(operation, payload) {
    const contract = operationContract(operation);
