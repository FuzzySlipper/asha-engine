...
import { cameraHandle, type CameraCreateRequest, type CollisionConstrainedCameraInputEnvelope } from '@asha/contracts';
...
const CAMERA_REQUEST: CameraCreateRequest = {
...
function encounterState(status: FpsEncounterStateReadout['status']): FpsEncounterStateReadout {
  return {
...
function rustEvidenceBridge(): {
  ...
  readonly calls: {
    readonly collision: CollisionConstrainedCameraInputEnvelope[];
    readonly fire: FpsPrimaryFireRequest[];
    readonly restart: FpsRuntimeSessionRestartRequest[];
    readonly encounterTransition: FpsEncounterTransitionRequest[];
  };
} {
  const base = createMockRuntimeBridge();
  const calls = {
    collision: [] as CollisionConstrainedCameraInputEnvelope[],
    fire: [] as FpsPrimaryFireRequest[],
    restart: [] as FpsRuntimeSessionRestartRequest[],
    encounterTransition: [] as FpsEncounterTransitionRequest[],
  };
  let epoch = 1;
  let player = 10;
  let enemy = 20;
  ...
  const bridge = new Proxy(base, {
    get(target, property, receiver) {
      if (property === 'applyCollisionConstrainedCameraInput') {
        return (input: CollisionConstrainedCameraInputEnvelope) => {
      ...
      if (property === 'applyFpsPrimaryFire') {
        return (request: FpsPrimaryFireRequest) => {
      ...
      if (property === 'readFpsEncounterDirector') {
        return (lifecycle: FpsEncounterLifecycleInput) => {
