...
export function voxelPaletteUpdateRequest(
...
export function createVoxelPaletteUpdateHandler(
  calls: string[],
): (handle: number, requestJson: string) => string {
  return (_handle, requestJson) => {
    calls.push(`voxelVolumeAssetPaletteUpdate:${requestJson}`);
    const request = JSON.parse(requestJson) as VoxelVolumeAssetPaletteUpdateRequest;
