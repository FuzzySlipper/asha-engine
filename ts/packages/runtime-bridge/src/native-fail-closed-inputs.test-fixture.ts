...
export const INPUT_SESSION_CONFIGURE_REQUEST: InputSessionConfigureRequest = {
  catalog: createDefaultBrowserInputCatalog(),
  initialContexts: ['gameplay'],
};

export const INPUT_CONTEXT_COMMAND: InputContextCommand = { operation: 'push', contextId: 'menu' };

export const RAW_INPUT_SAMPLE: RawInputSample = {
  sequence: 0,
...
export const RECORDED_INPUT_ACTION: RecordedInputAction = {
  schemaVersion: 1,
  action: {
    sequence: 0,
...
export function createNativeInputHandlers(hashA: string, hashB: string, hashC: string): Partial<NativeAddon> {
  return {
...
function timeControlState(mode: 'paused' | 'running', revision: number, authorityTick: number, stateHash: string) {
  return { schemaVersion: 1, mode, speedMultiplier: 1, revision, authorityTick, stateHash };
}

function inputContextState(revision: number, contextIds: readonly string[], stateHash: string) {
  return {
    schemaVersion: 1,
    revision,
...
export const MODEL_MATERIAL_PREVIEW_REQUEST: ModelMaterialPreviewRequest = {
  catalogEntry: {
    id: 'material.copper',
    kind: 'material',
    version: 1,
...
export const COLLISION_CAMERA_INPUT: CollisionConstrainedCameraInputEnvelope = {
  ...CAMERA_INPUT,
  grid: 1,
