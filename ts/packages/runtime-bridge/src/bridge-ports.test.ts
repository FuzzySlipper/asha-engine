...
void test('a simulation consumer can use a capability subset test double', () => {
  const calls: number[] = [];
  const simulation: RuntimeTimeSimulationPort = {
...
void test('every fixed port records lifecycle hash and resource rules', () => {
  assert.deepEqual(
    Object.keys(RUNTIME_BRIDGE_PORT_CONTRACTS),
    [
      'input',
      'timeSimulation',
      'sceneEntities',
