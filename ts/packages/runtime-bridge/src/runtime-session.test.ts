        ...
        return (request: FpsPrimaryFireRequest) => {
          ...
          return {
            ...
            lifecycleStatus: enemyShootsPlayer
              ...
              : { state: 'enemy_defeated' as const, entity: enemy, tick: request.tick },
...
void test('Rust-backed RuntimeSession routes ECRP load, primary fire, and restart through bridge authority', () => {
  ...
  assert.deepEqual(
    calls.load.at(-1)?.definitions.find((definition) => definition.role === 'enemy')?.policyBinding?.allowedIntents,
    ['runtime.intent.move_direct_nav.v0', 'runtime.intent.primary_fire.v0'],
  );
...
void test('Rust-backed RuntimeSession routes autonomous policy tick through bridge authority', () => {
  ...
  assert.throws(() => session.readNavProjection(), (error: unknown) => error instanceof RuntimeBridgeError && error.kind === 'operation_unimplemented');
  assert.throws(() => session.requestSessionRestart({ kind: 'runtime.restart_session_intent', source: 'programmatic', expectedSessionHash: '' }), (error: unknown) => error instanceof RuntimeBridgeError && error.kind === 'invalid_input');
});
