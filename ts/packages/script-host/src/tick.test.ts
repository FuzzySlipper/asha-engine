...
import { runPolicyTickStage, type PolicyTickStageInput } from './tick.js';
...
const labelAll = defineWorldPolicy('label-all', (v) =>
  v.entities.filter((e) => e.spatial).map((e) => worldCommands.addLabel(e.id, tagId(9))),
);
...
function input(over: Partial<PolicyTickStageInput> = {}): PolicyTickStageInput {
  return {
    tick: over.tick ?? 1,
    seed: over.seed ?? 100,
...
void test('the tick stage collects proposals from all policies in order with metadata', () => {
  ...
  assert.deepEqual(
    result.executions.map((e) => [e.policy, e.proposedCount]),
    [
      ['label-all', 2],
      ['noop', 1],
    ],
  ...
  assert.deepEqual(
    result.executions.map((e) => e.seed),
    [100, 101],
  );
});
