...
export const QUARANTINED_GLOBALS: readonly string[] = [
  'process',
  'fetch',
  'XMLHttpRequest',
  'WebSocket',
...
function throwingCapability(name: string): unknown {
  const fail = (): never => {
 ...
 */
export function runQuarantined<T>(fn: () => T): T {
  const g = globalThis as unknown as Mutable;
  const saved: { name: string; had: boolean; value: unknown }[] = [];
