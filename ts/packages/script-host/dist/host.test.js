...
void test('command collection preserves order across policies', () => {
    ...
    const result = invokePolicies([definePolicy('first', first), definePolicy('second', second)], makeView());
...
void test('a throwing policy yields a structured diagnostic, not a crash', () => {
    ...
    const result = invokePolicies([definePolicy('ok', ok), definePolicy('boom', boom)], makeView());
...
void test('collected() returns a defensive copy that cannot mutate the buffer', () => {
    ...
    snapshot.push(commands.deleteEntity(entityId(2)));
