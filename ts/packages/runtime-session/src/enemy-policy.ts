...
export interface GeneratedTunnelEnemyPolicyFixtureInput {
  ...
  readonly target: Partial<EnemyPolicyTargetView> & { readonly camera: CameraHandle };
  readonly nav: NavPolicyViewReadout;
...
export type EnemyPolicyFixtureNonClaim =
  | 'not_policy_runtime'
  | 'not_authority'
...
export type EnemyPolicyDiagnosticCode =
  | 'blocked_nav_path'
  | 'target_out_of_range'
...
export type EnemyPolicyForbiddenCapability =
  | 'clock'
  | 'random'
  | 'network'
  | 'dom'
...
interface ForbiddenCapabilityRule {
  readonly capability: EnemyPolicyForbiddenCapability;
  readonly token: string;
...
export function createGeneratedTunnelEnemyPolicyFixture(
  input: GeneratedTunnelEnemyPolicyFixtureInput,
): GeneratedTunnelEnemyPolicyFixture {
...
export function createEnemyPolicyView(input: GeneratedTunnelEnemyPolicyFixtureInput): EnemyPolicyView {
...
export function proposeEnemyPolicyFrame(view: EnemyPolicyView): EnemyPolicyProposalFrame {
  const diagnostics: EnemyPolicyDiagnostic[] = [];
  const proposals: EnemyPolicyProposal[] = [];
...
export function validateEnemyPolicySource(source: string): readonly EnemyPolicySourceDiagnostic[] {
  const diagnostics: EnemyPolicySourceDiagnostic[] = [];
...
function frameResult(
  tick: number,
  proposals: readonly EnemyPolicyProposal[],
  diagnostics: readonly EnemyPolicyDiagnostic[],
...
function distanceUnits(a: EnemyPolicyVec3, b: EnemyPolicyVec3): number {
  const dx = a[0] - b[0];
  const dy = a[1] - b[1];
  const dz = a[2] - b[2];
