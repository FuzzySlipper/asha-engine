...
import {
  GENERATED_TUNNEL_NAV_MARKER_CELLS,
...
void test('generated-tunnel encounter and nav projections correspond to source markers', () => {
  ...
  assert.deepEqual(
    ...
    projectGeneratedTunnelMarkerToNavCell(exitMarker),
  );
  assert.deepEqual(
    GENERATED_TUNNEL_NAV_MARKER_CELLS.player_start,
    projectGeneratedTunnelMarkerToNavCell(playerStartMarker),
  );
  assert.deepEqual(
    GENERATED_TUNNEL_REACHABLE_PATH.path[0],
    GENERATED_TUNNEL_NAV_MARKER_CELLS.exit_hint,
  );
  assert.deepEqual(
    GENERATED_TUNNEL_REACHABLE_PATH.path.at(-1),
    GENERATED_TUNNEL_NAV_MARKER_CELLS.player_start,
  );
});
