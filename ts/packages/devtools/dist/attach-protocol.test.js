...
import { ASHA_DEVTOOLS_PROTOCOL_VERSION, buildDevtoolsProtocolGoldenFixtures, createDevtoolsFixtureEndpoint, runDevtoolsProtocolConformance, } from './attach-protocol.js';
function clientType(message) {
    return message.type;
}
function serverType(message) {
    return message.type;
}
test('golden fixtures cover handshake and projection pull protocol messages', () => {
...
test('command proposal fixtures model accepted and rejected authority results', () => {
    const fixtures = buildDevtoolsProtocolGoldenFixtures();
...
test('evidence export is typed and carries artifact metadata', () => {
    const fixtures = buildDevtoolsProtocolGoldenFixtures();
...
test('stable protocol fixtures expose no methodName or anyJson catchall', () => {
    const fixtures = buildDevtoolsProtocolGoldenFixtures();
...
test('conformance harness passes against the in-process fixture endpoint', async () => {
...
test('conformance harness fails closed on protocol version mismatch', async () => {
...
test('conformance harness fails closed when command proposal support is missing', async () => {
...
//# sourceMappingURL=attach-protocol.test.js.map