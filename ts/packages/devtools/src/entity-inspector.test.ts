...
import {
  ...
  formatEntityInspector,
  type AuthoringEntityRecord,
} from './entity-inspector.js';

const eid = (n: number): EntityId => n as EntityId;

function record(over: Partial<AuthoringEntityRecord> & { id: EntityId }): AuthoringEntityRecord {
  return {
    lifecycle: 'active',
    hasTransform: false,
    hasRender: false,
...
function mixedRecords(): AuthoringEntityRecord[] {
  return [
