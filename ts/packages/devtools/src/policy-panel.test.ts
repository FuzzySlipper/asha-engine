...
import type { EntityId, PolicyWorldOutcome, PolicyWorldRejection, TagId } from '@asha/contracts';

import { buildPolicyRunSummary, formatPolicyRunSummary, type PolicyRunInput } from './policy-panel.js';

const eid = (n: number): EntityId => n as EntityId;
const tid = (n: number): TagId => n as TagId;

const accepted = (entity: EntityId, label: TagId): PolicyWorldOutcome => ({
  status: 'accepted',
  event: { kind: 'labelAdded', entity, label },
});
const rejected = (reason: PolicyWorldRejection): PolicyWorldOutcome => ({
  status: 'rejected',
  rejection: reason,
});

function run(over: Partial<PolicyRunInput> = {}): PolicyRunInput {
  return {
    tick: 7,
    executions: [
...
void test('an all-accepted run reports zero rejections and a stable replay handle', () => {
  const s = buildPolicyRunSummary(
    run({ outcomes: [accepted(eid(1), tid(9)), accepted(eid(2), tid(9))], replayHandle: 'tick-7-clean' }),
  );
