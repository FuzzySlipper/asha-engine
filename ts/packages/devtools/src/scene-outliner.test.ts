...
import {
  ...
  inspectNode,
  type OutlinerInput,
  type RuntimeEntityProjection,
} from './scene-outliner.js';
...
function at(x: number, y: number, z: number): SceneTransform {
  return { translation: [x, y, z], rotation: [0, 0, 0, 1], scale: [1, 1, 1] };
}

const meshAsset: AssetReference = { id: 'mesh:crate', version: { req: 'any' }, hash: null };

function node(over: Omit<Partial<SceneNodeRecord>, 'id'> & { id: number }): SceneNodeRecord {
  return {
...
function fixtureScene(): FlatSceneDocument {
  return {
    schemaVersion: 1,
    id: sceneId(1001),
...
function input(over: Partial<OutlinerInput> = {}): OutlinerInput {
  return {
...
void test('buildOutlinerModel builds a tree ordered by childOrder then id', () => {
  ...
  assert.equal(root.node.id as number, 10);
  // childOrder 0 (mesh-a, id 11) before childOrder 1 (group-b, id 12).
  assert.deepEqual(
    root.children.map((c) => c.node.id as number),
    [11, 12],
  );
  // group-b has the leaf.
  assert.deepEqual(
    root.children[1]!.children.map((c) => c.node.id as number),
    [13],
  );
});
...
void test('a scene node with a live source trace correlates as matched', () => {
  ...
  const mesh = model.roots[0]!.children.find((c) => (c.node.id as number) === 11)!;
...
void test('an orphaned node (absent parent) is shown explicitly with a diagnostic', () => {
  ...
  assert.deepEqual(
    model.orphans.map((o) => o.node.id as number),
    [20],
  );
...
void test('inspectEntity resolves a scene source and flags a dangling source', () => {
  ...
  assert.equal(resolved.sourceNode?.id as number, 11);
  assert.equal(resolved.danglingSource, false);

  const dangling = inspectEntity(
    input({ entities: [{ entityId: entityId(501), lifecycle: 'active', transform: null, sourceNode: sceneNodeId(99) }] }),
    entityId(501),
  )!;
