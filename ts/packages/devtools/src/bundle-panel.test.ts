...
import { createMockRuntimeBridge, RuntimeBridgeError, type RuntimeBridge } from '@asha/runtime-bridge';
import {
  sceneId,
  worldId,
...
function manifest(): WorldBundleManifest {
  return {
...
test('buildManifestModel flags a durable artifact missing its content hash', () => {
  const m = manifest();
  const broken: WorldBundleManifest = {
    ...m,
