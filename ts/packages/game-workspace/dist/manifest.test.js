...
import { ASHA_GAME_WORKSPACE_COMPATIBILITY, buildAshaAuthoringPersistenceContract, buildAshaGamePublishAssetManifest, parseAshaGameManifestToml, resolveAshaAuthoringWriteTarget, resolveAshaGameAssetForDev, validateAshaGameAssetCatalog, validateAshaConsumerCompatibility, } from './index.js';
...
void test('authoring write target resolver fails closed on disallowed paths and hatches', () => {
    ...
    assert.equal(!wrongExtension.ok && wrongExtension.diagnostics.some((diagnostic) => diagnostic.code === 'invalid_extension'), true);
    ...
    assert.equal(!privateTransport.ok && privateTransport.diagnostics.some((diagnostic) => diagnostic.code === 'private_transport_hint'), true);
});
