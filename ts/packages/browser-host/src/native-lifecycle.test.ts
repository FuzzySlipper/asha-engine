...
type FpsLoadRequest = Parameters<RuntimeBridge['loadFpsRuntimeSession']>[0];
...
void test('native browser host survives sustained movement defeat restart and invocation errors', async (t) => {
  ...
  try {
    ...
    const loaded = await invokeBridge<{ readonly sessionEpoch: number }>(
      host.url,
      'loadFpsRuntimeSession',
      [FPS_LOAD_REQUEST],
    );
    assert.equal(loaded.sessionEpoch, 1);
    const camera = await invokeBridge<{ readonly camera: number }>(host.url, 'createCamera', [{
    ...
    for (let cycle = 0; cycle < 64; cycle += 1) {
      const movement: { readonly collision: { readonly movementMode: string } } = await invokeBridge(
        host.url,
      ...
      const defeat: {
        ...
        readonly targetHealthAfter: { readonly current: number; readonly max: number } | null;
      } = await invokeBridge(
        host.url,
        'applyFpsPrimaryFire',
        [{
      ...
      const restarted: {
        ...
        readonly lifecycleStatus: { readonly state: string };
      } = await invokeBridge(host.url, 'restartFpsRuntimeSession', [{ expectedEpoch: epoch }]);
      epoch += 1;
      assert.equal(restarted.sessionEpoch, epoch);
      assert.equal(restarted.lifecycleStatus.state, 'active');
    }

    const rejected = await invokeBridgeResponse(
      host.url,
    ...
    const finalSnapshot = await invokeBridge<{
      readonly sessionEpoch: number;
      readonly lifecycleStatus: { readonly state: string };
    }>(host.url, 'readFpsRuntimeSession', []);
...
async function invokeBridgeResponse(
  baseUrl: string,
  method: string,
  args: readonly unknown[],
): Promise<{ readonly status: number; readonly result: unknown; readonly errorMessage: string }> {
  const response = await fetch(
    `${baseUrl}/asha/browser-host/runtime-bridge/${encodeURIComponent(method)}`,
    {
      method: 'POST',
