...
import {
  type GameplayRuntimeHostReadout,
...
import {
  ...
  ASHA_BROWSER_HOST_GAMEPLAY_METHODS,
  launchNativeBrowserHost,
} from './index.js';

const READOUT: GameplayRuntimeHostReadout = {
...
void test('browser host preserves the closed gameplay RuntimeSession transport', async () => {
  ...
  const calls: string[] = [];
  const gameplayHost: GameplayRuntimeHostTransport = {
    load() {
      calls.push('load');
      return {
  ...
  try {
    ...
    try {
      ...
      const provider = browserScope['ashaRuntimeBridge'] as {
        readonly gameplayHost: Record<string, unknown>;
      };
      ...
      assert.equal((advance['result'] as Record<string, unknown>)['accepted'], true);
...
async function invokeGameplayHost(
  baseUrl: string,
  method: string,
  args: readonly unknown[],
): Promise<Record<string, unknown>> {
  const response = await fetch(
    `${baseUrl}/asha/browser-host/gameplay-runtime-host/${encodeURIComponent(method)}`,
    {
      method: 'POST',
...
function createCompleteFakeBridge(): RuntimeBridge {
  const operation = () => ({ called: true }) as never;
  return Object.fromEntries(
    ASHA_BROWSER_HOST_BRIDGE_METHODS.map((method) => [method, operation]),
