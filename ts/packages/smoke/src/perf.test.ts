...
import { defaultBootBridge, type BridgeBoot } from './harness.js';
...
void test('reference perf run is structurally sound and all invariants hold', async () => {
  ...
  assert.ok(
    result.invariants.every((i) => i.held),
    `all invariants held: ${JSON.stringify(result.invariants)}`,
  );
