...
export function runSmoke(options: SmokeOptions = {}): SmokeResult {
  ...
  let editOk = false;
  try {
    const batch: CommandBatch = { commands: [{ kind: 'smoke-edit' }] };
    ...
    stages.push({
      ...
      ok: editOk,
      detail:
        `proposed 1 command → accepted=${result.accepted} rejected=${result.rejected}; ` +
    ...
    failures.push(
      classifyBridgeFailure('runtime-bridge.submitCommands', 'ui_command_rejected', cause),
    );
...
function classifyBridgeFailure(
  ...
  if (cause instanceof RuntimeBridgeError) {
    const category =
      cause.kind === 'native_unavailable' ? 'missing_native_bridge' : fallback;
    return {
      category,
      subsystem,
