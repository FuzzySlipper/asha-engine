...
export function proposeAttachTransformParent(child: EntityId, parent: EntityId): EntityAuthoringCommand {
  return { kind: 'attachTransformParent', child, parent };
}

/** Propose placing `member` inside `container` (logical containment, not transform). */
export function proposeSetContainment(member: EntityId, container: EntityId): EntityAuthoringCommand {
  return { kind: 'setContainment', member, container };
}

/** Propose recording a source-ancestry trace (read-only provenance, not a graph). */
export function proposeSetDerivedFrom(derived: EntityId, origin: EntityId): EntityAuthoringCommand {
 ...
 */
export function summarizeAuthoringOutcome(outcome: EntityAuthoringOutcome): AuthoringFeedback {
...
// ── Capability-class projection (pure, for devtools/outliner) ────────────────────
 ...
 */
export function classifyEntity(flags: EntityCapabilityFlags): readonly EntityClass[] {
 ...
 */
export function transformEligibility(flags: EntityCapabilityFlags): { readonly eligible: boolean; readonly reason: string | null } {
  if (flags.lifecycle === 'tombstoned') {
...
export function movementEligibility(flags: EntityCapabilityFlags): { readonly eligible: boolean; readonly reason: string | null } {
