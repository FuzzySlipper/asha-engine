...
import {
  ...
  proposeRenameSceneObject,
  proposeReparentSceneObject,
  sceneObjectIdForNode,
} from './scene-object-hierarchy.js';
...
function group(id: number, label: string, parent: number | null = null, childOrder = 0): SceneNodeRecord {
  return {
    id: sceneNodeId(id),
...
function doc(nodes: readonly SceneNodeRecord[]): FlatSceneDocument {
  return {
    schemaVersion: 1,
    id: sceneId(1001),
...
test('scene object snapshot projects flat scene document parent links and renderable provenance', () => {
  const snapshot = buildSceneObjectSnapshot({
    document: doc([group(1, 'Root'), group(2, 'Child', 1)]),
