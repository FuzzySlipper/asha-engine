...
function node(id: number, kind: SceneNodeKind, parent: number | null = null, childOrder = 0): SceneNodeRecord {
  return {
    id: sceneNodeId(id),
...
function baseScene(): FlatSceneDocument {
  return {
    schemaVersion: 1,
    id: sceneId(1001),
...
void test('proposeAddStaticMesh / Sprite / Group build typed addNode proposals', () => {
  ...
  if (mesh.op === 'addNode') {
    ...
    assert.equal(mesh.node.parent as number, 1);
...
void test('applyProposalToDraft applies add / reparent / setTransform / setMetadata', () => {
  ...
  const n2 = draft.nodes.find((n) => (n.id as number) === 2)!;
  assert.equal(n2.parent as number, 3);
...
function kindFromAssetId(id: string): 'mesh' | 'sprite' | 'other' {
  if (id.startsWith('mesh/')) return 'mesh';
  if (id.startsWith('sprite/')) return 'sprite';
  return 'other';
}

/** A faithful-enough mock of Rust scene validation for the end-to-end wiring test. */
function mockAuthorityValidate(doc: FlatSceneDocument): SceneValidationReport {
  const errors: SceneValidationError[] = [];
  const ids = new Set<number>(doc.nodes.map((n) => n.id as number));
  ...
  const parentOf = new Map<number, number | null>(doc.nodes.map((n) => [n.id as number, n.parent === null ? null : (n.parent as number)]));
  for (const startNode of doc.nodes) {
    const seen = new Set<number>();
    let cur: number | null = startNode.id as number;
    const path: number[] = [];
...
function submitProposal(doc: FlatSceneDocument, proposal: SceneEditProposal) {
