...
import { entityId, tagId, type PolicyAssetView, type PolicyEntityView } from '@asha/contracts';

import { deriveSummary, emptyWorldView, makeWorldView, worldQuery } from './world-view.js';

function spatial(id: number, label?: number): PolicyEntityView {
  return {
    id: entityId(id),
    lifecycle: 'active',
...
function logical(id: number, lifecycle: 'active' | 'disabled' = 'active'): PolicyEntityView {
...
void test('worldQuery reads entities, spatiality, labels, and asset status without mutating', () => {
  ...
  assert.deepEqual(
    worldQuery.spatialEntities(view).map((e) => e.id),
    [entityId(1)],
  );
  assert.deepEqual(
    worldQuery.activeEntities(view).map((e) => e.id),
