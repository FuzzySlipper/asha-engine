...
import { composeAppShell, formatReadout, } from './shell.js';
const FIXTURES = [
    {
        id: 'launch-grid',
        label: 'Launch grid',
...
void test('runtime mode readout distinguishes reference / degraded / unavailable', () => {
    // reference
    assert.equal(composeAppShell({ host: { name: 'h', accessibility: false }, bootBridge: referenceBoot, fixtures: FIXTURES })
        .runtimeStatus().availability, 'reference');
...
void test('fixture selection is runtime-selectable (data, not compile-time)', () => {
    ...
    assert.deepEqual(shell.fixtureListing().map((f) => ({ id: f.id, active: f.active })), [
...
void test('pick with no bridge clears selection and misses (fail closed)', () => {
    ...
    const ray = { grid: 1, origin: [0, 0, 0], direction: [1, 0, 0], maxDistance: 10 };
...
void test('empty fixture catalog is rejected at composition time', () => {
    assert.throws(() => composeAppShell({ host: { name: 'h', accessibility: false }, bootBridge: referenceBoot, fixtures: [] }), /non-empty fixture catalog/);
});
