...
import { audioHandle, } from '@asha/contracts';
import { AshaAudioHost, applyAshaRuntimeProjectionFrame, } from './audio-host.js';
...
void test('Web Audio host emits catalog-hash-bound 3D cues and caches decoded clips', async () => {
    ...
    const repeated = await audio.applyPresentation(frame([
        operation(0, {
            op: 'emit',
            signalId: 'shot:44',
            descriptor: descriptor(),
    ...
    const receipt = await audio.applyPresentation(frame([
        ...
        operation(1, {
            ...
            patch: {
                ...
                attenuation: null,
                pan: -0.5,
                emitter: { kind: 'entityAttached', entity: 5, offset: [1, 0, -1] },
            },
        }),
...
void test('missing audio host degrades after scene application with origin diagnostics', async () => {
    ...
    assert.equal(receipt.audio.diagnostics[0]?.origin?.id, 'combat.primary-fire.accepted:44');
});
...
void test('blocked AudioContext and malformed frame return explicit failures', async () => {
    ...
    let sceneApplied = false;
    await assert.rejects(applyAshaRuntimeProjectionFrame(frame([
        operation(1, {
            op: 'emit',
            signalId: 'bad-sequence',
            descriptor: descriptor(),
        }),
    ]), { applyScene: () => { sceneApplied = true; }, audioHost: audio }), /sequence must be contiguous/);
    assert.equal(sceneApplied, false, 'malformed outer framing rejects before scene');
});
//# sourceMappingURL=audio-host.test.js.map