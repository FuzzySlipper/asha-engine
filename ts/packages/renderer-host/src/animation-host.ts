...
interface AnimationControllerRealization {
  ...
  origin: PresentationOriginRef | null;
...
export class AshaAnimationHost {
  ...
  constructor(
    ...
    options: AshaAnimationHostOptions = {},
  ) {
  ...
  advance(deltaSeconds: number): AshaAnimationFrameReceipt {
    ...
    const diagnostics: AnimationProjectionDiagnostic[] = [];
    const cues: AshaAnimationSampledCue[] = [];
...
function validateCueDefinitions(
  definitions: readonly AshaAnimationClipCueDefinition[],
): readonly AshaAnimationClipCueDefinition[] {
...
function sampleAnimationCues(
  ...
  definitions: readonly AshaAnimationClipCueDefinition[],
  deltaSeconds: number,
): readonly AshaAnimationSampledCue[] {
  ...
  const sampled: AshaAnimationSampledCue[] = [];
...
function animationCueKey(definition: AshaAnimationClipCueDefinition): string {
