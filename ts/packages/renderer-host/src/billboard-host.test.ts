...
import {
  AshaBillboardHost,
  type AshaBillboardElement,
  type AshaBillboardElementStyle,
} from './billboard-host.js';
import { applyAshaRuntimeProjectionFrame } from './audio-host.js';

class FakeElement implements AshaBillboardElement {
  readonly style = {} as AshaBillboardElementStyle;
...
class FakeContainer {
  ...
  appendChild(element: AshaBillboardElement): void {
...
void test('a missing billboard host is isolated after scene application', async () => {
  let sceneApplied = 0;
  const frame: RuntimeProjectionFrame = {
    schemaVersion: 1,
    authorityTick: 9,
    scene: { ops: [] },
