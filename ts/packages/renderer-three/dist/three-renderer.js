// Retained Three.js scene projector for ASHA render diffs.
import * as THREE from 'three';
import { decodeRenderFrameDiff, RuntimeBridgeError, } from '@asha/runtime-bridge';
import { AnimatedMeshApplyError, AnimatedMeshRegistry, } from './animated-mesh.js';
...
export class ThreeRenderer {
    ...
    #createStaticMeshInstance(diff) {
        ...
        const ownedMaterialIndices = new Set();
    ...
    #createAnimatedMeshInstance(diff) {
        ...
        const parent = diff.parent === null ? this.#sceneGroup : this.#require(diff.parent, 'createAnimatedMeshInstance.parent').object;
    ...
    #syncAnimatedMeshPlayback(handle, entry) {
    ...
    #replaceLiveMaterial(id) {
        const replacedSharedMaterials = new Set();
    ...
    #setMaterialInstanceParameters(diff) {
        ...
        if (entry.kind !== 'staticMesh' || entry.asset === undefined || entry.materialIds === undefined) {
            throw new RenderApplyError(`setMaterialInstanceParameters: handle ${diff.handle} is not a static-mesh instance`);
        }
        const def = this.#staticMeshes.get(entry.asset);
        const index = def?.slotIndex.get(diff.slot);
        if (def === undefined || index === undefined) {
            throw new RenderApplyError(`setMaterialInstanceParameters: unbound slot ${diff.slot} on ${entry.asset}`);
        }
        const materialId = entry.materialIds[index];
        if (materialId === null || materialId === undefined) {
            throw new RenderApplyError(`setMaterialInstanceParameters: slot ${diff.slot} on ${entry.asset} has no material`);
        }
        const mesh = entry.object;
        ...
        if (diff.parameters === null) {
            ...
            materials[index] = this.#materialFor({ slot: diff.slot, material: materialId }, diff.parameters);
            entry.ownedMaterialIndices?.add(index);
        }
...
function snapshotLine(handle, entry) {
    ...
    if (entry.kind === 'animatedMesh') {
        const playback = o.userData['animatedMeshPlayback'] ?? null;
        return [
            head,
            `kind animatedMesh`,
            `asset ${entry.asset}`,
...
function disposeInstanceMaterials(entry) {
    const mesh = entry.object;
    const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    entry.ownedMaterialIndices?.forEach((index) => materials[index]?.dispose());
}
function standardMaterial(descriptor, parameters) {
    ...
    const color = new THREE.Color(descriptor.color[0] * tint[0], descriptor.color[1] * tint[1], descriptor.color[2] * tint[2]);
