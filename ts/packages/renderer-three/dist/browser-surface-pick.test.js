...
import { entityId, renderHandle, tagId } from '@asha/contracts';
...
function pickAtCenter(renderer) {
    const camera = new THREE.PerspectiveCamera(70, 1, 0.05, 100);
    camera.position.set(0, 0, 0);
    camera.lookAt(0, 0, -1);
    camera.updateMatrixWorld(true);
    return pickProjectedObject(renderer, camera, new THREE.Raycaster(), new THREE.Vector2(), {
...
void test('projection pick filters are descriptive only and invalid rays fail closed', () => {
    ...
    renderer.applyFrame(pickFrame());
    const beforeSnapshot = renderer.snapshot();
    const filtered = pickProjectedObject(renderer, new THREE.PerspectiveCamera(70, 1, 0.05, 100), new THREE.Raycaster(), new THREE.Vector2(), {
    ...
    const rejected = pickProjectedObject(renderer, new THREE.PerspectiveCamera(70, 1, 0.05, 100), new THREE.Raycaster(), new THREE.Vector2(), { ray: { kind: 'viewport', point: [1.01, 0] } });
    assert.equal(rejected.hit, null);
    assert.equal(rejected.diagnostics[0]?.code, 'invalid_viewport_point');
    const oversizedFilter = pickProjectedObject(renderer, new THREE.PerspectiveCamera(70, 1, 0.05, 100), new THREE.Raycaster(), new THREE.Vector2(), {
...
//# sourceMappingURL=browser-surface-pick.test.js.map