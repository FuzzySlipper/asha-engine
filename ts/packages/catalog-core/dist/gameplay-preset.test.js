...
void test('default FPS gameplay preset validates and exposes stable readout references', () => {
    ...
    assert.equal(report.readout.hashes.referenceHash, 'fnv1a64:aff976e05786ce21');
});
...
void test('FPS gameplay preset validator rejects arbitrary payload hatches and unexpected keys', () => {
    ...
    assert.equal(report.valid, false);
    assert.equal(report.readout, null);
    assert.deepEqual(report.diagnostics.map((diagnostic) => diagnostic.code), ['arbitraryPayloadRejected', 'arbitraryPayloadRejected']);
...
void test('FPS gameplay preset catalog readout lists consumer ownership and stable hashes', () => {
    ...
    assert.equal(catalog.hashes.catalogHash, 'fnv1a64:45a696e1e10c562f');
    assert.equal(catalog.hashes.defaultPresetHash, 'fnv1a64:b39b8794318889a7');
});
...
//# sourceMappingURL=gameplay-preset.test.js.map